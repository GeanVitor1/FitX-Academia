    ...
    <div class="recepcao-dashboard">
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          <div class="stat-icon">💰</div>
          <div class="stat-info">
            <span class="stat-value">{{ pagamentosHoje() }}</span>
            <span class="stat-label">Pagamentos Hoje</span>
          </div>
        </div>
        ...
        <div class="stat-card">
          <div class="stat-icon">📋</div>
          <div class="stat-info">
            <span class="stat-value">{{ novosAlunos() }}</span>
            <span class="stat-label">Novos Alunos</span>
          </div>
        </div>
      </div>
      ...
      <div class="main-content">
            ...
            @if (foundStudent()) {
              <div class="found-student">
                ...
                <button class="checkin-btn" (click)="doCheckin()">
                  {{ foundStudent()!.checkedIn ? 'Check-out' : 'Check-in' }}
                </button>
              </div>
            }
          </div>
        ...
        <div class="present-section">
          <div class="section-header">
            ...
            <span class="count">{{ presentList.length }} alunos</span>
          </div>
          <div class="present-list">
            @for (student of presentList; track student.id) {
              <div class="present-item">
                ...
                <div class="student-info">
                  <h4>{{ student.name }}</h4>
                  <p>Entrou às {{ student.entryTime }}</p>
                </div>
                <button class="checkout-btn" (click)="doCheckout(student.id)">Saída</button>
              </div>
            }
          </div>
        </div>
      ...
      <div class="recent-section">
        <div class="section-header">
          ...
          <a routerLink="/recepcao/historico" class="view-all">Ver histórico →</a>
        </div>
        <div class="recent-list">
          @for (checkin of recentCheckins; track checkin.id) {
            <div class="recent-item">
              ...
              <span class="status-badge" [class]="checkin.status">
                {{ checkin.status === 'in' ? 'Entrada' : 'Saída' }}
              </span>
            </div>
          }
        </div>
...
export class RecepcaoComponent {
  ...
  presentes = signal(0);
  pagamentosHoje = signal(0);
  novosAlunos = signal(0);

  foundStudent = signal<{ id: string; name: string; plan: string; checkedIn: boolean } | null>(null);

  presentList: { id: string; name: string; entryTime: string }[] = [];

  recentCheckins: { id: string; name: string; time: string; status: string }[] = [];

  private allStudents: { id: string; name: string; plan: string; checkedIn: boolean }[] = [];
  ...
  doCheckin(): void {
    ...
    if (student) {
      ...
      this.toast.success(student.checkedIn ? 'Check-in realizado!' : 'Check-out realizado!');
    }
  }

  doCheckout(studentId: string): void {
    this.presentList = this.presentList.filter(s => s.id !== studentId);
    this.presentes.update(v => v - 1);
  }
}

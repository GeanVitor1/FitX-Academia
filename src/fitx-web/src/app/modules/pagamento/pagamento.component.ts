...
import { MensalidadeDto } from '../../core/models/models';
...
@Component({
    ...
    <div class="pagamento-page">
      ...
      @if (mensalidades().length > 0) {
        <div class="current-plan">
          ...
          <div class="plan-info">
            <h2>{{ mensalidades()[0].planoNome }}</h2>
            <p class="plan-price">R$ {{ mensalidades()[0].valor.toFixed(2) }}/mes</p>
          </div>
          <div class="plan-status">
            <span class="status-badge" [class.active]="mensalidades()[0].status === 'Paga'" [class.pending]="mensalidades()[0].status !== 'Paga'">
              {{ mensalidades()[0].status === 'Paga' ? 'Em dia' : 'Pendente' }}
            </span>
            <p>Vencimento: {{ mensalidades()[0].dataVencimento | date:'dd/MM/yyyy' }}</p>
          </div>
        </div>
              ...
              <div class="pix-content">
                <div class="pix-info">
                  ...
                  <div class="info-row">
                    ...
                    <span class="info-value highlight">R$ {{ mensalidades()[0].valor.toFixed(2) }}</span>
                  </div>
                </div>
          ...
          <div class="history-list">
            @for (m of mensalidades(); track m.id) {
              <div class="history-item">
                <div class="payment-status" [class.paid]="m.status === 'Paga'" [class.pending]="m.status !== 'Paga'">
                  {{ m.status === 'Paga' ? 'OK' : '...' }}
                </div>
                <div class="payment-info">
                  <span class="payment-desc">{{ m.planoNome }}</span>
                  <span class="payment-date">{{ m.dataVencimento | date:'dd/MM/yyyy' }}</span>
                </div>
                <div class="payment-amount">
                  <span class="amount">R$ {{ m.valor.toFixed(2) }}</span>
                  <span class="status-text" [class.paid]="m.status === 'Paga'" [class.pending]="m.status !== 'Paga'">
  ...
  styles: [`
    ...
    .current-plan { display: flex; align-items: center; gap: 1.5rem; padding: 1.25rem; background: #111113; border: 1px solid #1e1e22; border-radius: 12px; margin-bottom: 1.5rem; }
    .plan-badge { padding: 4px 10px; background: rgba(200, 255, 0, 0.1); color: #c8ff00; border-radius: 4px; font-size: 10px; font-weight: 700; letter-spacing: 0.05em; }
    .plan-info { flex: 1; }
    ...
    .status-badge.pending { background: rgba(234, 179, 8, 0.12); color: #fbbf24; }
    .plan-status p { font-size: 11px; color: #52525b; margin: 4px 0 0; }
    .payment-methods { display: flex; gap: 8px; margin-bottom: 1.5rem; }
    .method-btn { flex: 1; display: flex; flex-direction: column; align-items: center; gap: 6px; padding: 1rem; background: #111113; border: 1px solid #1e1e22; border-radius: 10px; cursor: pointer; transition: all 0.2s; }
    ...
    .payment-section { background: #111113; border: 1px solid #1e1e22; border-radius: 12px; padding: 1.5rem; margin-bottom: 1.5rem; }
    ...
    .qr-code { width: 200px; height: 200px; background: #0a0a0a; border: 1px solid #1e1e22; border-radius: 12px; padding: 16px; display: flex; align-items: center; justify-content: center; }
    .qr-code svg { width: 100%; height: 100%; }
    .qr-text { font-size: 12px; color: #52525b; margin: 0; }
    .pix-copy { display: flex; align-items: center; gap: 8px; width: 100%; padding: 8px 12px; background: #18181b; border: 1px solid #1e1e22; border-radius: 8px; }
    .pix-code { flex: 1; font-size: 9px; color: #52525b; word-break: break-all; font-family: monospace; }
    .copy-btn { padding: 4px 12px; background: #18181b; border: 1px solid #1e1e22; border-radius: 4px; color: #c8ff00; font-size: 11px; font-weight: 600; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
    ...
    .form-input { width: 100%; padding: 10px 12px; background: #18181b; border: 1px solid #1e1e22; border-radius: 8px; color: #fafafa; font-size: 13px; }
    ...
    .btn-primary { width: 100%; padding: 10px; background: #c8ff00; color: #000; border: none; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
    .btn-primary:hover { background: #d4ff33; }
    .btn-secondary { width: 100%; padding: 10px; background: #18181b; border: 1px solid #1e1e22; border-radius: 8px; color: #a1a1aa; font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.2s; }
    ...
    .history-item { display: flex; align-items: center; gap: 1rem; padding: 12px 16px; background: #111113; border: 1px solid #1e1e22; border-radius: 10px; transition: border-color 0.2s; }
    .history-item:hover { border-color: #3f3f46; }
    .payment-status { width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 6px; font-size: 10px; font-weight: 700; }
    ...
    .empty-state { text-align: center; padding: 2rem; background: #111113; border: 1px solid #1e1e22; border-radius: 12px; }
    .empty-state p { font-size: 13px; color: #52525b; margin: 0; }
  `]
})
    ...
    this.alunosService.getByUsuarioId(user.id).subscribe({
      next: (res) => {
        if (res.success && res.data) {
          this.mensalidadesService.getByAlunoId(res.data.id).subscribe({
            next: (r) => { if (r.success && r.data) this.mensalidades.set(r.data); this.loading.set(false); },
